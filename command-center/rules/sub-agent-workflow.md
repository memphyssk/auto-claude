...
1. **Read `command-center/Sub-agent Instructions/<agent-name>-instructions.md`** and inject the contents as the FIRST directive in the spawn prompt. Positive forward-looking directives the agent reads.
2. If the instruction file is missing, create an empty stub. **Hard rule:** only create files for agents that exist in the available agent list — never invent agent names.
