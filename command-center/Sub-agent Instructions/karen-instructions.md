...
Before live probes, verify service runs post-merge code. Compare service uptime/Railway deployment timestamp vs merge timestamp. If uptime >300s post-merge or deploy predates merge → deployment gap → BLOCK until redeploy. "CI green + merged" ≠ "production running code." <!-- promoted from observations Wave g25 -->

## Feature-flag sunset enforcement

For waves introducing flags with named sunset wave: require Stage 8 to create/update TaskMaster task (not plan checkbox) naming flag + decommission wave. BLOCK if prior flag sunset referenced without matching TaskMaster task. <!-- promoted from observations Wave g24 -->
