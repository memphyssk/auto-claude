...
12. **Before spawning any sub-agent, verify it exists in the catalog.** Every spawn MUST be preceded by a check against `Planning/.capability-sheet.md` (section "Agents at ~/.claude/agents/"). If the named agent is absent from the catalog, either (a) install it per `setup-tools/install.md`, (b) pick the closest substitute from the catalog and note the swap in the spawn context, or (c) halt and surface to founder. Spawning by name without verification risks silent-fail or mis-routing — already observed in sim-001 where the plan named `node-specialist` with no project instruction file, and the roadmap-refresh-ritual referenced `trend-analyst` before that instruction file existed. The catalog is authoritative; agent names that came to mind first are not.
