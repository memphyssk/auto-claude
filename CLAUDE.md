...
| User says "danger builder" / "ship it mode" / "ceo mode" / "run indefinitely" / "full delegation" / "365 mode" | `command-center/management/mode-switching.md` (sets `mode: danger-builder`) + `command-center/management/danger-builder-mode.md` (prerequisite checks + ceo-agent resolution of BOARD splits/HARD-STOPs + per-decision Resend notifications + indefinite loop) + `command-center/management/ceo-bound.md` (founder-authored charter — restrictions only; silent = unlimited) + `command-center/Sub-agent Instructions/ceo-agent-instructions.md` (personality + decision procedure) + `command-center/management/notifications/resend.md` (per-decision email spec). **Verifies prerequisites before entry — charter exists, Resend env vars set, kill-switch mechanism works. One email per CEO decision. Founder reached via kill-switch, session message, or notification email.** |
